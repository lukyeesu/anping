...
const splitIndex = v3Content.indexOf('function doGet(e)');
const originalAfterDoGet = v3Content.substring(splitIndex);

const newDoPostAndLineLogic = `// ==========================================
...
const v4Content = newDoPostAndLineLogic + originalAfterDoGet;

fs.writeFileSync(path.join(projectDir, 'หลังบ้าน v.4 LINE.js'), v4Content);
console.log('Successfully created v.4 with accurate string splitting!');
